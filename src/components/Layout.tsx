  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col md:flex-row">
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-slate-900 border-t border-slate-800 flex justify-around p-3 z-50">
        <MobileNavItem icon={<Calendar />} label="Home" active />
        <MobileNavItem icon={<Tent />} label="Festivals" />
        <MobileNavItem icon={<Users />} label="Trips" />
      </div>
    </div>
  );
}
...
function MobileNavItem({ icon, label, active = false }: { icon: React.ReactElement; label: string; active?: boolean }) {
    return (
